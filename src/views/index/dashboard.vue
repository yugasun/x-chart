    ...
    >
        ...
        >
            <grid-item
                ...
                :i="item.i"
                :ref="item.i"
                :style="gridItemBackground(item.chartType, colorStart, colorEnd)"
...
export default class Dashboard extends Vue {
    ...
    gridDraggable: boolean = true;
    gridResizable: boolean = true;
    gridColNum: any = 36;
    gridRowHeight: any = 10;
    gridMargin: any[] = [30, 30, 30, 30];
    cachePosition: any = {
        x: 0,
        y: 0,
        w: 0,
     ...
     */
    layoutUpdating: boolean = false;
    slices: any = []; // charts modules
    ...
    gridItemBackground(type: string, start: string, stop: string) {
        return {
            background:
                type === "x-number"
     ...
     */
    calPopPosition(expandTarget: any) {
        ...
        const height = Math.round(
            this.gridRowHeight * popH +
                Math.max(0, popH - 1) * this.gridMargin[1]
        );
        ...
        const popY =
            (top - this.gridMargin[1]) /
            (this.gridRowHeight + this.gridMargin[1]);
     ...
     */
    restorePosition(item: SliceItem) {
     ...
     * go into focus mode
     */
    handleFocus({ expand, targetRef }: ExpandPayload) {
        ...
        if (expand) {
            ...
            this.$nextTick(() => {
                ...
                this.slices = this.slices.map((item: SliceItem) => {
                    if (item.i === targetRef) {
                        // 1. 记录当前位置信息
